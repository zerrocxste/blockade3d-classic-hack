...
void CDrawing::AddCircle3D(Vector position, Matrix4x4 matrix, float points, float radius, ImColor& color)
{
	...
	{
		Vector start;
		start.x = radius * cosf(a) + position.x;
		start.y = position.y;
		start.z = radius * sinf(a) + position.z;

		Vector end;
		end.x = radius * cosf(a + step) + position.x;
		end.y = position.y;
		end.z = radius * sinf(a + step) + position.z;

		float start2d[2], end2d[2];

		if (!world_to_screen(matrix, start, start2d) || !world_to_screen(matrix, end, end2d))
			return;

		AddLine(ImVec2(start2d[0], start2d[1]), ImVec2(end2d[0], end2d[1]), color);
	}
}
...
void CDrawing::AddText(float x, float y, const ImColor& color, float fontSize, int font_style, int flags, const char* format, ...)
{
	if (!format)
		return;
